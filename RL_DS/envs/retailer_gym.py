...
class RetailerOrdersEnv(gym.Env):
    ...
    def __init__(self, time_horizon=47, track_data=False, window=47):
        super(RetailerOrdersEnv, self).__init__()

        # Store parameters
        self.time_horizon = time_horizon
        self.track_data = track_data
        self.dem_window=window
        ...
        # Gym Spaces
        obs_size= self.dem_window + 7 ## ventana de conocimiento de demanda + 7 observaciones
    ...
    def _get_observation(self):
        ...
        supply_gap = self.cumulative_customer_demand - self.cumulative_shipment
        demand_profile = np.zeros(self.dem_window, dtype=np.float32)
        
        for step_idx in range(self.dem_window):
...
# weeks=47
# env= RetailerOrdersEnv(time_horizon=weeks, track_data=True)
...
class RetailerOrdersEnv2(gym.Env):
    """
    Gymnasium environment for simulating retailer orders and supply chain dynamics.
    Tracks step-by-step information if track_data=True.
    """
    ...
    def __init__(self, time_horizon=47, track_data=False, window=1):
        super(RetailerOrdersEnv2, self).__init__()
    ...
    def _get_observation(self):
        ...
        demand_profile= [self.initial_channel_demand + self._pulse(self.current_step)]
...
# weeks=47
# env= RetailerOrdersEnv2(time_horizon=weeks, track_data=True)
